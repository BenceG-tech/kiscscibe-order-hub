...
export const CHANGELOG: ChangelogEntry[] = [
  {
    ...
    description:
      "A második átvizsgálási körben 5 új, valós hibaforrást találtunk arra, hogy 'nem érkezik meg a rendelés az adminra' vagy 'nem jön az email'. 1) IDEMPOTENCIA UNIQUE INDEX: eddig két gyors kérés között ugyanaz az idempotencia-kulcs elméletileg két külön rendelés-sort hozhatott létre (kettős email, kétszer levont adag). Most partial unique index védi, sose lehet duplikátum. 2) ABANDONED_CARTS UNIQUE: a session_id-re nem volt egyediségi kényszer, ezért a szerveroldali upsert csendben több sort hozott létre — most egy session = egy sor, sokkal tisztább 'megkísérelt rendelés' napló. 3) REALTIME ROBUSZTUS: az admin rendeléslista most 30 másodpercenként polling-fallback-kal is frissül, alvó laptop/wifi-váltás/proxy timeout után automatikusan újracsatlakozik a websocket, és ablak-fókusz visszavétel azonnal újratölt — soha többé nem tűnhet el egyetlen rendelés sem az admin nézetből. 4) EMAIL FELADÓ DOMAIN JAVÍTVA: eddig a feladó 'rendeles@kiscsibe-etterem.hu' (kötőjeles) volt, ami ha nem verifikált a Resendben, csendben 403-mal elhalt — átírtuk 'rendeles@kiscsibeetterem.hu'-ra (a bejáratott domain), és minden email sikeres/hibás státusza mostantól rögzül az admin diagnosztikába. 5) REGGELI ÁTVÉTELI ABLAK: a reggeli-only kosaraknál most már valóban aktiválódik a 07:00–10:00 átvételi ablak (eddig a tag hiánya miatt sose lépett érvénybe).",
    type: "fixed",
    tabGroup: "orders",
  },
  {
