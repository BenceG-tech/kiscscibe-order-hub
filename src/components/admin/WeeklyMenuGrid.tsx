...
const CATEGORY_COLORS: Record<string, { row: string; sticky: string }> = {
  ...
  "Főzelék feltét": { row: "bg-lime-100 dark:bg-lime-950/30", sticky: "bg-lime-100 dark:bg-lime-950" },
  ...
  "Tokány / Pörkölt / Ragu": { row: "bg-rose-50 dark:bg-rose-950/30", sticky: "bg-rose-50 dark:bg-rose-950" },
};
...
export default function WeeklyMenuGrid() {
  ...
  const clearDayMutation = useMutation({
    mutationFn: async (date: string) => {
      const offer = dailyOffers.find(o => o.date === date);
      if (!offer) return;
      
      const itemIds = offer.daily_offer_items?.map(i => i.id) || [];
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["daily-offers-week"] });
      toast.success("Nap ürítve");
    },
    onError: () => {
      toast.error("Hiba történt a nap ürítésekor");
    },
  });
